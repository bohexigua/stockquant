...
# 配置日志 - 输出到文件和控制台
log_filename = os.path.join(logs_dir, f'stock_cyq_daily_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
logging.basicConfig(
    level=logging.INFO,
...
class StockCyqDailyCleaner:
    ...
    def _check_date_exists(self, trade_date: str) -> bool:
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = """
                SELECT COUNT(*) 
                FROM trade_market_stock_cyq_daily 
    ...
    def fetch_cyq_data_range(self, start_date: str, end_date: str) -> pd.DataFrame:
        """获取指定日期范围的股票每日筹码数据（单日循环获取并立即入库）"""
        try:
            logger.info(f"开始获取股票每日筹码数据，日期范围: {start_date} - {end_date}")
            ...
            for trade_date in trading_dates:
                try:
                    ...
                    logger.info(f"正在获取{trade_date}的股票每日筹码数据")
                    
                    # 获取单日数据
                    df_cyq = self.tushare_api.cyq_perf(trade_date=trade_date)
                    
                    if not df_cyq.empty:
                        logger.info(f"成功获取{trade_date}的{len(df_cyq)}条数据")
                        ...
                        # 清洗数据
                        df_cleaned = self.clean_cyq_data(df_cyq, df_basic)
                        
                        if not df_cleaned.empty:
                            # 立即入库
                            success = self.insert_cyq_data(df_cleaned)
                            if success:
            ...
            logger.error(f"获取股票每日筹码数据失败: {e}")
            return pd.DataFrame()
    
    def clean_cyq_data(self, df_cyq: pd.DataFrame, df_basic: pd.DataFrame) -> pd.DataFrame:
        """清洗股票每日筹码数据"""
        if df_cyq.empty:
            return df_cyq
        ...
        try:
            # 合并股票基础信息以获取股票名称
            df_merged = df_cyq.merge(df_basic, on='ts_code', how='left')
            ...
            # 处理空值和数据类型
            numeric_columns = ['his_low', 'his_high', 'cost_5pct', 'cost_15pct', 'cost_50pct', 
            ...
            logger.info(f"筹码数据清洗完成，清洗后数据量: {len(df_cleaned)}")
            return df_cleaned
            
        except Exception as e:
            logger.error(f"筹码数据清洗失败: {e}")
            return pd.DataFrame()
    
    def insert_cyq_data(self, df: pd.DataFrame) -> bool:
        """将股票每日筹码数据插入数据库"""
        if df.empty:
            logger.warning("没有筹码数据需要插入")
            return False
        ...
        try:
            with self.connection.cursor() as cursor:
                # 构建插入SQL
                sql = """
                INSERT INTO trade_market_stock_cyq_daily 
                (trade_date, code, name, his_low, his_high, cost_5pct, cost_15pct, 
                ...
                logger.info(f"成功插入{len(data_list)}条股票每日筹码数据到数据库")
                return True
                
        except Exception as e:
            logger.error(f"插入股票每日筹码数据失败: {e}")
            return False
    
    def update_cyq_data(self) -> bool:
        """更新股票每日筹码数据"""
        try:
            start_date, end_date = self.get_trading_date_range()
            logger.info(f"开始更新股票每日筹码数据，日期范围: {start_date} - {end_date}")
            
            # fetch_cyq_data_range 方法已经包含了数据获取、清洗和入库的完整流程
            result_df = self.fetch_cyq_data_range(start_date, end_date)
            
            if not result_df.empty and 'total_count' in result_df.columns:
                total_count = result_df['total_count'].iloc[0]
                if total_count > 0:
                    logger.info(f"股票每日筹码数据更新完成，共处理{total_count}条数据")
                    return True
                else:
                    logger.info("所有数据已存在或无新数据需要处理")
                    return True
            else:
                logger.warning("股票每日筹码数据更新失败")
                return False
            
        except Exception as e:
            logger.error(f"更新股票每日筹码数据失败: {e}")
            return False
    
    def get_cyq_data_by_date(self, trade_date: str, limit: int = 10) -> pd.DataFrame:
        """查询指定日期的股票每日筹码数据"""
        try:
            with self.connection.cursor() as cursor:
                sql = """
                SELECT trade_date, code, name, his_low, his_high, cost_5pct, cost_15pct,
                       cost_50pct, cost_85pct, cost_95pct, weight_avg, winner_rate
                FROM trade_market_stock_cyq_daily
                WHERE trade_date = %s
                ...
                if results:
                    ...
                    logger.info(f"查询到{len(df)}条股票每日筹码数据")
                    return df
                else:
                    logger.info(f"未查询到{trade_date}的股票每日筹码数据")
                    return pd.DataFrame()
                    
        except Exception as e:
            logger.error(f"查询股票每日筹码数据失败: {e}")
            return pd.DataFrame()
...
def main():
    ...
    try:
        # 创建清洗器实例
        cleaner = StockCyqDailyCleaner()
        
        success = cleaner.update_cyq_data()
        
        if success:
            logger.info("股票每日筹码数据处理完成")
        else:
            logger.error("股票每日筹码数据处理失败")
