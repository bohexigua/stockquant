...
class StockDailyCleaner:
    ...
    def fetch_daily_data_range(self, start_date: str, end_date: str) -> pd.DataFrame:
        """获取指定日期范围的股票日行情数据（单日循环获取并立即入库）"""
        try:
            logger.info(f"开始获取股票日行情数据，日期范围: {start_date} - {end_date}")
            ...
            for trade_date in trading_dates:
                try:
                    ...
                    logger.info(f"正在获取{trade_date}的股票日行情数据")
                    
                    # 获取单日数据
                    df_daily = self.tushare_api.daily(trade_date=trade_date)
                    ...
                    if not df_daily.empty:
                        ...
                        # 清洗数据
                        df_cleaned = self.clean_daily_data(df_daily, df_basic)
                        
                        if not df_cleaned.empty:
                            # 立即入库
                            success = self.insert_daily_data(df_cleaned)
                            if success:
    ...
    def clean_daily_data(self, df_daily: pd.DataFrame, df_basic: pd.DataFrame) -> pd.DataFrame:
        """清洗股票日行情数据"""
        if df_daily.empty:
            return df_daily
        ...
        try:
            ...
            # 处理空值和数据类型
            numeric_columns = ['open', 'high', 'low', 'close', 'pre_close', 'chg_val', 'chg_pct', 'amount']
    ...
    def insert_daily_data(self, df: pd.DataFrame) -> bool:
        """将股票日行情数据插入数据库"""
        if df.empty:
        ...
        try:
            with self.connection.cursor() as cursor:
                # 构建插入SQL
                sql = """
                INSERT INTO trade_market_stock_daily 
                (trade_date, code, name, open, high, low, close, pre_close, 
                 chg_val, chg_pct, vol, amount)
                ...
                logger.info(f"成功插入{len(data_list)}条股票日行情数据到数据库")
                return True
                
        except Exception as e:
            logger.error(f"插入股票日行情数据失败: {e}")
            return False
    ...
    def update_daily_data(self) -> bool:
        ...
        try:
            ...
            logger.info(f"开始更新股票日行情数据，日期范围: {start_date} - {end_date}")
            
            # fetch_daily_data_range 方法已经包含了数据获取、清洗和入库的完整流程
            result_df = self.fetch_daily_data_range(start_date, end_date)
            
            if not result_df.empty and 'total_count' in result_df.columns:
                total_count = result_df['total_count'].iloc[0]
                if total_count > 0:
                    logger.info(f"股票日行情数据更新完成，共处理{total_count}条数据")
                    return True
                else:
                    logger.info("所有数据已存在或无新数据需要处理")
                    return True
            else:
                logger.warning("股票日行情数据更新失败")
                return False
            
        except Exception as e:
            logger.error(f"更新股票日行情数据失败: {e}")
            return False
    
    def get_daily_data_by_date(self, trade_date: str, limit: int = 10) -> pd.DataFrame:
        """查询指定日期的股票日行情数据"""
        try:
            with self.connection.cursor() as cursor:
                sql = """
                SELECT trade_date, code, name, open, high, low, close, 
                ...
                if results:
                    ...
                    logger.info(f"查询到{len(df)}条股票日行情数据")
                    return df
                else:
                    logger.info(f"未查询到{trade_date}的股票日行情数据")
                    return pd.DataFrame()
                    
        except Exception as e:
            logger.error(f"查询股票日行情数据失败: {e}")
            return pd.DataFrame()
...
def main():
    ...
    try:
        # 创建清洗器实例
        cleaner = StockDailyCleaner()
        
        success = cleaner.update_daily_data()
        
        if success:
            logger.info("股票日行情数据处理完成")
        else:
            logger.error("股票日行情数据处理失败")
