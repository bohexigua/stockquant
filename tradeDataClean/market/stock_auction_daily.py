...
# 配置日志 - 输出到文件和控制台
log_filename = os.path.join(logs_dir, f'stock_auction_daily_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
logging.basicConfig(
    level=logging.INFO,
...
class StockAuctionDailyCleaner:
    """股票开盘集合竞价数据清洗器"""
    ...
    def fetch_auction_data_range(self, start_date: str, end_date: str) -> pd.DataFrame:
        """获取指定日期范围的股票开盘集合竞价数据（单日循环获取并立即入库）"""
        try:
            logger.info(f"开始获取股票开盘集合竞价数据，日期范围: {start_date} - {end_date}")
            ...
            logger.error(f"获取股票开盘集合竞价数据失败: {e}")
            return pd.DataFrame()
    ...
    def clean_auction_data(self, df_auction: pd.DataFrame, df_basic: pd.DataFrame) -> pd.DataFrame:
        """清洗股票开盘集合竞价数据"""
        if df_auction.empty:
            return df_auction
    ...
    def insert_auction_data(self, df: pd.DataFrame) -> bool:
        """将股票开盘集合竞价数据插入数据库"""
        if df.empty:
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                logger.info(f"成功插入{len(data_list)}条股票开盘集合竞价数据到数据库")
                return True
                
        except Exception as e:
            logger.error(f"插入股票开盘集合竞价数据失败: {e}")
            return False
    
    def update_auction_data(self) -> bool:
        """更新股票开盘集合竞价数据"""
        try:
            start_date, end_date = self.get_trading_date_range()
            logger.info(f"开始更新股票开盘集合竞价数据，日期范围: {start_date} - {end_date}")
            
            # fetch_auction_data_range 方法已经包含了数据获取、清洗和入库的完整流程
            result_df = self.fetch_auction_data_range(start_date, end_date)
            
            if not result_df.empty and 'total_count' in result_df.columns:
                total_count = result_df['total_count'].iloc[0]
                if total_count > 0:
                    logger.info(f"股票开盘集合竞价数据更新完成，共处理{total_count}条数据")
                    return True
                else:
                    logger.info("所有数据已存在或无新数据需要处理")
                    return True
            else:
                logger.warning("股票开盘集合竞价数据更新失败")
                return False
            
        except Exception as e:
            logger.error(f"更新股票开盘集合竞价数据失败: {e}")
            return False
    
    def get_auction_data_by_date(self, trade_date: str, limit: int = 10) -> pd.DataFrame:
        """查询指定日期的股票开盘集合竞价数据"""
        try:
            with self.connection.cursor() as cursor:
                sql = """
                ...
                if results:
                    ...
                    logger.info(f"未查询到{trade_date}的股票开盘集合竞价数据")
                    return pd.DataFrame()
                    
        except Exception as e:
            logger.error(f"查询股票开盘集合竞价数据失败: {e}")
            return pd.DataFrame()
...
def main():
    ...
    try:
        # 创建清洗器实例
        cleaner = StockAuctionDailyCleaner()
        
        success = cleaner.update_auction_data()
        
        if success:
            logger.info("股票开盘集合竞价数据处理完成")
        else:
            logger.error("股票开盘集合竞价数据处理失败")
