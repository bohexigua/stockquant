...
class BuyStrategy:
    ...
    def write_strategy_evaluation(self, code: str, stock_name: str, decision_side: str, will_execute: int, summary: str, execute_qty: int = 0):
        try:
            with self.db.cursor() as c:
                c.execute(
                    "INSERT INTO ptm_quant_strategy_evaluations (trade_date, stock_code, stock_name, decision_side, will_execute, summary, related_strategy, execute_qty) "
                    "VALUES (CURDATE(), %s, %s, %s, %s, %s, %s, %s) "
                    "ON DUPLICATE KEY UPDATE summary=VALUES(summary), execute_qty=VALUES(execute_qty), updated_time=CURRENT_TIMESTAMP",
                    (code, stock_name, decision_side, will_execute, summary, STRATEGY_NAME, execute_qty),
                )
        except Exception:
            pass
    ...
    def decide_buy(self, code: str, cash_before: float, stock_name: str, now_dt: datetime = None):
        ...
        if qty_to_buy <= 0:
            self.write_strategy_evaluation(code, stock_name, 'BUY', 0, '仓位不足')
            return None
        ...
        self.write_strategy_evaluation(code, stock_name, 'BUY', 1, reason, qty_to_buy)
