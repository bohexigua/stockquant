    ...
    try:
        with strategy.db.cursor() as c:
            ...
            if now_t >= time(9, 0, 0):
                ...
                tdate = drow[0]
            c.execute(
                "SELECT trade_time, price, pre_close, volume FROM trade_market_stock_tick WHERE code=%s AND trade_date=%s AND trade_time<='11:00:00' ORDER BY trade_time DESC LIMIT 1",
                (code, tdate),
            )
            trow = c.fetchone()
            if not trow:
