...
def run_backtest():
    ...
    # 添加策略优化
    cerebro.optstrategy(StrongSectorLowStockArbitrageStrategy, 
                       max_rank=[30, 50])
    ...
    for i, result in enumerate(results):
        ...
        print(f"sharpe_ratio: {hasattr(strat.analyzers, 'sharpe')}, {hasattr(strat.analyzers.sharpe, 'get_analysis')}, {sharpe_analysis}")        
        # 从策略实例获取参数
        max_rank = strat.params.max_rank if hasattr(strat.params, 'max_rank') else 'N/A'
        
        print(f"参数组合 {i+1}: max_rank={max_rank}, 收益率={return_pct:.2f}%, 夏普比率={sharpe_ratio:.4f}, 最终资金={final_value:,.0f}")
    ...
    if best_result:
        strat = best_result[0]
        best_max_rank = strat.params.max_rank if hasattr(strat.params, 'max_rank') else 'N/A'
        
        # 计算最佳策略的收益率
        best_final_value = strat.broker.getvalue() if hasattr(strat, 'broker') else initial_cash
        best_return_pct = ((best_final_value / initial_cash) - 1) * 100 if initial_cash > 0 else 0
